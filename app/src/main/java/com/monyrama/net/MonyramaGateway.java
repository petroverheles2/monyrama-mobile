...
public class MonyramaGateway {
	private static final int PROTOCOL_VERSION = 2;
	
	private static final int SO_TIMEOUT = 10000;
	private static final int CONNECTION_TIMEOUT = 5000;
	
	private static MonyramaGateway instance = new MonyramaGateway();
	private Context context;
	
	private MonyramaGateway() {
		
	}
	
	public static MonyramaGateway getInst(Context context) {
		instance.context = context;
		return instance;
	}
	
	public MonyramaGatewayResponse getDataFromServer(Server server) {
		MonyramaGatewayResponse pbvpResponse;
		...
		try {
			...
			if(response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				...
				pbvpResponse = MonyramaGatewayResponse.createSuccessResponse(payload);
			} else {
				...
				pbvpResponse = MonyramaGatewayResponse.createErrorResponse(errorMessage);
			}
		} catch (Exception e) {
			...
			pbvpResponse = MonyramaGatewayResponse.createConnectionErrorResponse(message);
		}		
	...
	private String fetchErrorMessage(HttpResponse response) throws IOException,
		...
		return IOUtils.toString(errorInput);
	}
	
	public MonyramaGatewayResponse sendDataToServer(Server server, String json) {
		MonyramaGatewayResponse pbvpResponse;
		...
		try {
			...
			pbvpResponse = MonyramaGatewayResponse.createErrorResponse(context.getString(R.string.unexpected_error));
		} 
		...
		try {			
			...
			if(response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				pbvpResponse = MonyramaGatewayResponse.createSuccessResponse("");
			} else {
				String errorMessage = fetchErrorMessage(response);
				if(StringUtil.emptyString(errorMessage)) {
					errorMessage = context.getResources().getString(R.string.unexpected_response_from_server);
