...
class SavedCommentDAOImpl extends PBVPDatabaseOpenHelper implements SavedCommentDAO {

	private static final int MAX_SAVED_COMMENTS_NUMBER = 100;

	SavedCommentDAOImpl(Context context) {
		super(context);
	}
	...
	public void saveIfNew(String comment) {
		...
		Cursor cursor = db.query(SAVED_COMMENTS, new String[]{COMMENT_ID_COLUMN, COMMENT_COLUMN}, COMMENT_COLUMN + "=?", new String[]{comment}, null, null, null);
		if(!cursor.moveToFirst()) {			
			...
			db.insert(SAVED_COMMENTS, null, values);			
		} else {
			//Update timestamp of comment
			Integer commentid = cursor.getInt(cursor.getColumnIndex(COMMENT_ID_COLUMN));
			ContentValues values = new ContentValues();
			values.put(TS_COLUMN, new Date().getTime());
			db.update(SAVED_COMMENTS, values, COMMENT_ID_COLUMN + "=?", new String[]{commentid.toString()});			
		}
	...
	public List<String> getAll() {
		...
		Cursor countCursor = db.rawQuery("SELECT count(*) FROM " + SAVED_COMMENTS, null);
		...
		if(count > MAX_SAVED_COMMENTS_NUMBER) {
			...
			int rows = db.delete(SAVED_COMMENTS, TS_COLUMN + " < ?", new String[]{Long.toString(new Date().getTime() - thirtyDaysInMillis)});
			MyLog.debug(rows + " of unused saved comments deleted");
		}

		//Get saved comments
		Cursor cursor = db.query(SAVED_COMMENTS, new String[]{COMMENT_COLUMN}, null, null, null, null, null);
