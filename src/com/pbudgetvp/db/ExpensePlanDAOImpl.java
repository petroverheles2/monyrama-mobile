...
class ExpensePlanDAOImpl extends PBVPDatabaseOpenHelper implements
	...
	@Override
	public List<Envelope> getAllItemsByExpensePlan(ExpensePlan expensePlan) {
		SQLiteDatabase db = getReadableDatabase();
	...
	private ExpensePlan mapExpensePlan(Cursor cursor) {
		...
		expensePlan.setSum(new BigDecimal(cursor.getInt(cursor.getColumnIndex(SUM_COLUMN))).divide(new BigDecimal("100")));
		expensePlan.setRemainder(new BigDecimal(cursor.getString(cursor.getColumnIndex(REMAINDER_COLUMN))).divide(new BigDecimal("100")));
		return expensePlan;
	}
	...
	private Envelope mapEnvelope(Cursor cursor) {
		...
		item.setSum(new BigDecimal(cursor.getInt(cursor.getColumnIndex(SUM_COLUMN))).divide(new BigDecimal("100")));
		item.setRemainder(new BigDecimal(cursor.getInt(cursor.getColumnIndex(REMAINDER_COLUMN))).divide(new BigDecimal("100")));
